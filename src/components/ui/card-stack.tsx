            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-[2.5rem] border-4 border-black/5 dark:border-white/10 overflow-hidden shadow-2xl",
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6 md:p-10">
        <div className="text-2xl md:text-3xl font-bold font-display text-white line-clamp-1">
          {item.title}
        </div>
        {item.description ? (
          <div className="mt-2 line-clamp-2 text-sm md:text-lg text-white/90 font-medium">
            {item.description}
          </div>
        ) : null}
      </div>
