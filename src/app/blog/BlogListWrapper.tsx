...
import { Calendar, Clock, ArrowRight, Mail, Search, Share2, Bookmark } from 'lucide-react';
import { urlFor } from '@/sanity/lib/image';

const categories = ["All Updates", "Health Tips", "Medical News", "Lab Updates", "Technology"];

export default function BlogListWrapper({ initialPosts }: { initialPosts: any[] }) {
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-32 relative z-20 pb-32">
        ...
        {/* Featured Post Card */}
        {featuredPost && !searchQuery && activeCategory === "All Updates" && (
          <motion.div 
          ...
          >
            <div className="flex flex-col lg:flex-row gap-10 items-stretch">
              <div className="w-full lg:w-1/2 aspect-[16/10] overflow-hidden rounded-[2.8rem] relative bg-gray-50">
                {featuredPost.image && (
                   <img 
              ...
              <div className="w-full lg:w-1/2 flex flex-col justify-center py-6 px-4 md:px-8">
                <div className="flex items-center gap-6 mb-8 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-primary" /> {formatDate(featuredPost.publishedAt)}</span>
            ...
            {filteredPosts.map((post, index) => (
              ...
              >
                <Link href={`/blog/${post.slug}`} className="flex flex-col flex-grow">
                  <div className="aspect-[4/3] overflow-hidden rounded-[2.5rem] relative mb-8 shadow-lg bg-gray-50">
                    {post.image && (
                       <img 
                  ...
                  <div className="px-2 flex flex-col flex-grow">
                    <div className="flex items-center gap-4 mb-4 text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
                      <span className="flex items-center gap-1.5"><Calendar size={12} className="text-primary" /> {formatDate(post.publishedAt)}</span>
        ...
        >
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <form className="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto bg-gray-soft p-2 rounded-[2rem] border border-gray-100" onSubmit={(e) => e.preventDefault()}>
              <input 
